...
mod tests {
    ...
    fn test_pext_pdep_inverse() {
        // PEXT 和 PDEP 是互逆操作
        let mask = 0b10110100u32;
        for value in 0..16u32 {
            let deposited = pdep32(value, mask);
            let extracted = pext32(deposited, mask);
            assert_eq!(extracted, value, "value={}", value);
        }
    }

    #[test]
    fn test_pext_basic() {
        // source = 0b11111111, mask = 0b10101010
        // 提取 bit 1, 3, 5, 7 → 结果 = 0b1111
        assert_eq!(pext64(0b11111111, 0b10101010), 0b1111);

        // source = 0b10101010, mask = 0b10101010
        // 提取 bit 1, 3, 5, 7 → 结果 = 0b1111
        assert_eq!(pext64(0b10101010, 0b10101010), 0b1111);

        // source = 0b01010101, mask = 0b10101010
        // 提取 bit 1, 3, 5, 7（都是 0）→ 结果 = 0b0000
        assert_eq!(pext64(0b01010101, 0b10101010), 0b0000);
    }

    #[test]
    fn test_pdep_basic() {
        // source = 0b1111, mask = 0b10101010
        // 将 4 个 bit 存放到位置 1, 3, 5, 7
        assert_eq!(pdep64(0b1111, 0b10101010), 0b10101010);

        // source = 0b0101, mask = 0b10101010
        // bit 0 → pos 1, bit 2 → pos 5
        assert_eq!(pdep64(0b0101, 0b10101010), 0b00100010);
    }

    #[test]
    fn test_deposit_mask() {
        // 2 bits → 3 bits，新 bit 在位置 0
        assert_eq!(compute_deposit_mask(2, 0), 0b110);

        // 2 bits → 3 bits，新 bit 在位置 1
        assert_eq!(compute_deposit_mask(2, 1), 0b101);

        // 2 bits → 3 bits，新 bit 在位置 2
        assert_eq!(compute_deposit_mask(2, 2), 0b011);

        // 边界情况
        assert_eq!(compute_deposit_mask(0, 0), 0);
        assert_eq!(compute_deposit_mask(1, 0), 0b10);
        assert_eq!(compute_deposit_mask(1, 1), 0b01);
    }

    #[test]
    fn test_compression_mask() {
        // 3 bits，移除位置 0
        assert_eq!(compute_compression_mask(3, 0), 0b110);

        // 3 bits，移除位置 1
        assert_eq!(compute_compression_mask(3, 1), 0b101);

        // 3 bits，移除位置 2
        assert_eq!(compute_compression_mask(3, 2), 0b011);
    }

    #[test]
    fn test_expand_partial_keys() {
        ...
        // 2 bits → 3 bits，新 bit 插入位置 1
        // deposit_mask = 0b101
        // 0b00 → 0b000
        // 0b01 → 0b001
        // 0b10 → 0b100
        // 0b11 → 0b101
        expand_partial_keys(&mut keys, 4, 0b101);

        assert_eq!(keys[0], 0b000);
        assert_eq!(keys[1], 0b001);
        assert_eq!(keys[2], 0b100);
        assert_eq!(keys[3], 0b101);
    }

    #[test]
    fn test_compress_partial_keys() {
        let mut keys = [0u32; 32];
        keys[0] = 0b000;
        keys[1] = 0b001;
        keys[2] = 0b100;
        ...
        // 3 bits → 2 bits，移除位置 1
        // compression_mask = 0b101
        compress_partial_keys(&mut keys, 4, 0b101);

        assert_eq!(keys[0], 0b00);
        assert_eq!(keys[1], 0b01);
        assert_eq!(keys[2], 0b10);
        assert_eq!(keys[3], 0b11);
    }
    ...
    fn test_hw_sw_consistency() {
        ...
        // 测试硬件和软件实现的一致性
        for src in 0..256u64 {
            for mask in 1..256u64 {
                let hw_pext = unsafe { std::arch::x86_64::_pext_u64(src, mask) };
                let sw_pext = pext64_soft(src, mask);
                assert_eq!(hw_pext, sw_pext, "PEXT mismatch: src={}, mask={}", src, mask);

                let hw_pdep = unsafe { std::arch::x86_64::_pdep_u64(src, mask) };
                let sw_pdep = pdep64_soft(src, mask);
                assert_eq!(hw_pdep, sw_pdep, "PDEP mismatch: src={}, mask={}", src, mask);
            }
        }
    }
}
